...
def knot_spacing_and_variance(signal, times, quality, min_dt=None, max_dt=None, verbose=False, measurement_variance=1e-3):
    ...
    variance_e = dt_to_variance_spectrum(Xhat, freqs, dt)

    H = spline_interpolation_response(freqs, dt)
    variance_n = measurement_variance * signal_energy(H) / len(Xhat)

    print("Noise std part n(t): ",np.sqrt(variance_n), " fitting error std e(t): ",np.sqrt(variance_e))

    variance = variance_n + variance_e
    return dt, variance, variance_n, variance_e
